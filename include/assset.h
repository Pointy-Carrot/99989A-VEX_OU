...
typedef struct __attribute__((__packed__)) _assset {
  uint8_t *buf;
  size_t size;
} assset;
...
#define ASSSET(x)                                                            \
  extern uint8_t _binary_static_##x##_start[], _binary_static_##x##_size[]; \
  static assset x = {_binary_static_##x##_start, (size_t)_binary_static_##x##_size};

#endif

#define DECODE_PNG(var, assset, width, height) \
  var.data = new uint32_t[width * height];    \
  vexImagePngRead(assset.buf, &var, width, height, assset.size);

#define DECODE_BMP(var, assset, width, height) \
  var.data = new uint32_t[width * height];    \
  vexImageBmpRead(assset.buf, &var, width, height);
